...
##
## Project adaptation:
##   PicoRV32 + signed INT8 dot-product accelerator self-test
## Top-level module:
##   pynqz2_accel_top
##
## Active board resources:
##   sysclk  : H16, 125 MHz PL clock
##   btn_rst : D19, BTN0, active high
##   led[0]  : R14, firmware test completed
##   led[1]  : P14, firmware test passed
##   led[2]  : N16, PicoRV32 trapped
##   led[3]  : M14, firmware test failed
##   uart_tx : V6, Raspberry Pi header physical pin 8 (RPIO14)
##   uart_rx : Y6, Raspberry Pi header physical pin 10 (RPIO15)
##
## All unused constraints from the TUL master file remain commented so that
## the physical pin source stays auditable.
...
# BTN0 is asynchronous and is sampled only by the first stage of a two-flop
# synchronizer. The synchronized value controls a fully synchronous SoC reset.
set_false_path -from [get_ports { btn_rst }]

# LEDs are human-observable status outputs and have no external synchronous
# receiver. Their board-level setup/hold timing is therefore not applicable.
set_false_path -to [get_ports { led[*] }]

# Three-wire 3.3 V UART on the Raspberry Pi header. uart_rx is asynchronous;
# only the path into the first flip-flop of uart_mmio's two-stage synchronizer
# is excluded. uart_tx has no source-synchronous external timing requirement.
